...
export function buildDiscoveryRestUrl(
  ...
  const url = new URL(
    `https://us-central1-${projectId}.cloudfunctions.net/api/v1/discovery/deck`
  );
...
export function mapDiscoveryRestProfiles(payload: Record<string, unknown>): DiscoveryProfile[] {
  const candidates = Array.isArray(payload.profiles)
    ...
    : Array.isArray(payload.candidates)
      ? payload.candidates
      : [];
  const profiles: Array<DiscoveryProfile | null> = candidates
    .map((candidate) => {
      ...
      return {
        ...
        photos,
        distance: typeof source.distance_km === 'number'
          ? source.distance_km
        ...
        lastActive: typeof source.last_active === 'string'
          ? source.last_active
  ...
  return profiles.filter(
    (candidate): candidate is DiscoveryProfile =>
      candidate !== null && candidate.id.length > 0
  );
}
